...
from pytube.botGuard import bot_guard

from pytube.async_http_client import AsyncHTTPClient
...
class AsyncYouTube:
    ...
    def __repr__(self):
        return f'<pytube.async_main.AsyncYouTube object: videoId={self.video_id}>'
    ...
    async def get_js(self):
        ...
        if pytube.__js_url__ != js_url:
            self._js = await self.http_client.get(js_url)
            pytube.__js__ = self._js
            pytube.__js_url__ = js_url
        else:
    ...
    async def get_vid_info(self):
        ...
        if not self._vid_info:
            raise pytube.exceptions.InnerTubeResponseError(self.video_id, self.client)
        return self._vid_info
    ...
    async def caption_tracks(self) -> List[pytube.Caption]:
    ...
    async def captions(self) -> pytube.CaptionQuery:
        """Interface to query caption tracks.

        :rtype: :class:`CaptionQuery <CaptionQuery>`.
        """
        caption_tracks = await self.caption_tracks()
        return pytube.CaptionQuery(caption_tracks)
    

    async def chapters(self) -> List[pytube.Chapter]:
    ...
    async def key_moments(self) -> List[pytube.KeyMoment]:
